...
@section('this-page-contain')
    <div class="container-fluid px-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h2 class="mb-1">Data Mu'allim</h2>
            </div>
        </div>

        {{-- Breadcrumb --}}
        <div class="bg-light p-2 mb-3 border rounded small">
            <span>Data Pokok / <span class="text-success fw-semibold">Mu'allim</span></span>
        </div>

        {{-- Search Filter --}}
        <div class="card shadow-sm mb-4 border-0">
            <div class="card-body">
                <form action="{{ route('pokok.muallim.index') }}" method="GET"
                    ...
                    @if (!empty($search))
                        <a href="{{ route('pokok.muallim.index') }}" class="btn btn-secondary d-flex align-items-center">
                ...
                <div class="col-12">
                    <div class="card shadow-sm border-0">
                        <div class="card-body text-center py-5">
                            ...
                            <p class="text-muted mt-3 mb-0">Belum ada data Mu'allim ditemukan.</p>
                        </div>
                    </div>
                </div>
            @endforelse
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mt-2 mb-5">
            <div class="text-muted small mb-3 mb-md-0">
                Menampilkan <span class="fw-bold">{{ $muallim->firstItem() ?? 0 }}</span>
                sampai <span class="fw-bold">{{ $muallim->lastItem() ?? 0 }}</span>
                dari total <span class="fw-bold text-success">{{ $muallim->total() }}</span> data
            </div>

            @if ($muallim->hasPages())
                <div>
                    {{ $muallim->appends(request()->query())->links('pagination::bootstrap-5') }}
                </div>
            @endif
        </div>
