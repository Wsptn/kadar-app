                            ...
                            @forelse ($pendidikan as $index => $item)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if (!Auth::user()->isDaerah())
                                            <div class="btn-group">
                                                <a href="{{ route('master.pendidikan.edit', $item->id) }}"
                                                    class="btn btn-outline-warning btn-sm">
                                                    <i data-feather="edit-2" style="width: 14px;"></i>
                                                </a>
                                                <form action="{{ route('master.pendidikan.destroy', $item->id) }}" class="d-inline" method="POST" onsubmit="return confirm('Hapus data ini?')">
                                                    @csrf @method('DELETE')
