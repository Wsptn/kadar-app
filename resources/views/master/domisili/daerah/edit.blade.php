...
@section('this-page-contain')
    <div class="container-fluid px-4">
        <div class="d-flex justify-content-between align-items-center mb-4 mt-4">
            <div>
                <h2 class="fw-bold mb-0">Edit Daerah</h2>
            </div>
        ...
        <div class="card shadow-sm border-0">
            <div class="card-body">
                ...
                <form action="{{ route('master.domisili.daerah.update', $daerah->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select name="wilayah_id" class="form-select @error('wilayah_id') is-invalid @enderror" required>
                            ...
                            @foreach ($wilayah as $w)
                                <option value="{{ $w->id }}"
                                    {{ old('wilayah_id', $daerah->wilayah_id) == $w->id ? 'selected' : '' }}>
                                    {{ $w->nama_wilayah }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold">Nama Daerah</label>
                        <input type="text" name="nama_daerah"
                            class="form-control @error('nama_daerah') is-invalid @enderror"
                            value="{{ old('nama_daerah', $daerah->nama_daerah) }}" required>
                        @error('nama_daerah')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
